    ...
    public class GameLoadingWindow : XNAWindow
    {
        ...
        private void BtnDelete_LeftClick(object sender, EventArgs e)
        {
            ...
            var msgBox = new XNAMessageBox(WindowManager, "删除确认",
                    "将会删除以下存档：" + Environment.NewLine +
                    Environment.NewLine +
                    "文件名：" + sg.FileName + Environment.NewLine +
                    "存档名称：" + Renderer.GetSafeString(sg.GUIName, lbSaveGameList.FontIndex) + Environment.NewLine +
                    "日期时间：" + sg.LastModified.ToString() + Environment.NewLine +
                    Environment.NewLine +
                    "真的要继续吗？", XNAMessageBoxButtons.YesNo);
            msgBox.Show();
