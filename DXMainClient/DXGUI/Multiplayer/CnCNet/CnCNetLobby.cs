...
namespace DTAClient.DXGUI.Multiplayer.CnCNet
{
    ...
    internal class CnCNetLobby : XNAWindow, ISwitchable
    {
        ...
        public override void Initialize()
        {
            ...
            btnGameSortAlpha.SetToolTipText("按名称排序");
            ...
            btnGameFilterOptions.SetToolTipText("筛选房间");
        ...
        private void PostUIInit()
        {
            ...
            connectionManager.MainChannel.AddMessage(new ChatMessage(Color.White, Renderer.GetSafeString(
                    "由Shimada Mizuki（ https://github.com/enderseven1 ）汉化并修改", lbChatMessages.FontIndex)));
        ...
        private void ConnectionManager_BannedFromChannel(object sender, ChannelEventArgs e)
        {
            ...
            if (game == null)
            {
                ...
                chatChannel?.AddMessage(new ChatMessage(Color.White, $"无法加入频道{chatChannel.UIName}，你已被封禁"));
                return;
            }

            connectionManager.MainChannel.AddMessage(new ChatMessage(Color.White, $"无法加入房间{game.RoomName}，你已被房主封禁"));
        ...
        {
            if (isJoiningGame)
                return "无法加入房间 - 房间正在进行游戏，如果是服务器出错，请退出登录并重连再试。";
        ...
        private string GetJoinGameError(HostedCnCNetGame hg)
        {
            if (hg.Game.InternalName.ToUpper() != localGameID.ToUpper())
                return "选择的房间是" + gameCollection.GetGameNameFromInternalName(hg.Game.InternalName) + "的";
        ...
        {
            connectionManager.MainChannel.AddMessage(new ChatMessage(Color.White, "选择的房间已锁定"));
        ...
        private void AddMessageToChat(ChatMessage message)
        {
            ...
            {
                lbChatMessages.AddMessage(new ChatMessage(Color.Silver, "屏蔽信息 - 来自" + message.SenderName));
            }
            else
            {
        ...
        private void JoinUser(IRCUser user, IMessageView messageView)
        {
            ...
            if (game == null)
            {
                messageView.AddMessage(new ChatMessage(Color.White, user.Name + "不在房间内"));
                return;
            }
