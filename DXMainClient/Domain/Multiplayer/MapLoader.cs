...
namespace DTAClient.Domain.Multiplayer
{
    public class MapLoader
    {
        ...
        public Map LoadCustomMap(string mapPath, out string resultMessage)
        {
            ...
            {
                Logger.Log("LoadCustomMap: 找不到地图" + mapPath);
                resultMessage = $"地图文件{mapPath}{MAP_FILE_EXTENSION}不存在";

                return null;
            }

            Logger.Log("LoadCustomMap: 加载自定义地图" + mapPath);
            ...
            if (map.SetInfoFromCustomMap())
            {
                foreach (GameMode gm in GameModes)
                {
                    ...
                    {
                        Logger.Log("LoadCustomMap: 自定义地图" + mapPath + "已加载");
                        resultMessage = $"地图{mapPath}已加载";

                        return null;
                    }
                }

                Logger.Log("LoadCustomMap: 地图" + mapPath + "添加成功");
                ...
                resultMessage = $"地图{mapPath}加载成功";

                return map;
            }

            Logger.Log("LoadCustomMap: 加载地图" + mapPath + "失败");
        ...
        {
            Logger.Log("正在删除地图" + gameModeMap.Map.Name);
        ...
        private void AddMapToGameModes(Map map, bool enableLogging)
        {
            foreach (string gameMode in map.GameModes)
            {
                ...
                foreach (string gameModeAlias in gameModeAliases)
                {
                    ...
                    gm.Maps.Add(map);
                    if (enableLogging)
                        Logger.Log("AddMapToGameModes: 已添加地图" + map.Name + "到" + gm.Name + "模式");
                }
            }
        }
    }
