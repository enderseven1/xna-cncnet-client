...
> **注意**
> 微软没给编码加`&#`和`;`，你知道加上就行了，你学他只会报错。因为程序将数字识别为字符，多位数会被识别为多个字符，而`<Start>`和`<End>`只能包含单个字符。使用数字编码必须带上`&#`和`;`。
> 也就是说你不能写成这样：
> ``` xml
> <Start>32</Start><End>126</End>
> ```

但是Unicode使用十六进制数字，而每次都十六进制和十进制转来转去很麻烦，可以直接用十六进制数表示区间。区间最好不要有带本节图二的红叉的字符，我之前因为这个报错过，但是后面做好像又不报错，不确定。
...
``` xml
<!-- 注意：空格也会被识别为字符，也就是说你不小心在某个字符前面混了个空格进去也是会报错的。-->
<Start> </Start><End>~</End>
```
...
当然，如果搞不好会更炸裂。通过这种方式生成的粗体一般都没有间距，需要在`<Spacing>`和`</Spacing>`中设置1/2来保持间距，不然会糊成一坨，**接着把字体样式从Bold改成Regular（因为本身就是加粗的字体不用再用Bold加粗一遍）**。
...
- 将大部分汉字注释掉，只留英文数字符号和少量汉字（1000个字符以下），但是这种办法会导致很多`?`不便观察（但是你可以挑几个没注释的汉字改到UI里或者打到聊天框里）。
